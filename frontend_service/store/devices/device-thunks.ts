...
>("devices/fetchAll", async (_, thunkAPI) => {
  try {
    ...
    const response = await axios.get(`${DEVICE_BASE}/get-all-devices`, {
      headers,
    });
    thunkAPI.dispatch(
      setDevices({
...
>("devices/create", async (payload, thunkAPI) => {
  try {
    const headers = buildAuthHeaders(thunkAPI.getState());
    const response = await axios.post(`${DEVICE_BASE}/create-device`, payload, {
      headers,
    });
  ...
  async ({ device_id, dto, refreshMode = "all" }, thunkAPI) => {
    try {
      ...
      const response = await axios.put(
        `${DEVICE_BASE}/update-device/${device_id}`,
        dto,
        { headers }
      );
      if (refreshMode === "all") {
...
>("devices/delete", async ({ device_id }, thunkAPI) => {
  try {
    ...
    const response = await axios.delete(
      `${DEVICE_BASE}/delete-device/${device_id}`,
      {
        headers,
      }
    );
...
>("devices/getUsersForDevice", async ({ device_id }, thunkAPI) => {
  try {
    ...
    const response = await axios.get(
      `${DEVICE_BASE}/get-users-by-device/${device_id}`,
      {
        headers,
      }
    );
...
>("devices/linkToUser", async ({ device_id, user_id }, thunkAPI) => {
  try {
    ...
    const response = await axios.post(
      `${DEVICE_BASE}/link-device-to-user/${device_id}`,
      null,
      {
        params: { user_id },
        headers,
...
>("devices/unlinkFromUser", async ({ device_id, user_id }, thunkAPI) => {
  try {
    const headers = buildAuthHeaders(thunkAPI.getState());
    const response = await axios.delete(
      `${DEVICE_BASE}/unlink-device-from-user/${device_id}`,
      {
        params: { user_id },
        headers,
      }
...
>("devices/fetchByUser", async ({ user_id }, thunkAPI) => {
  try {
    const headers = buildAuthHeaders(thunkAPI.getState());
    const response = await axios.get(
      `${DEVICE_BASE}/get-devices-by-user/${user_id}`,
      {
        headers,
      }
    );
    ...
    const deviceResponses = await Promise.all(
      deviceIds.map((id) =>
        axios.get(`${DEVICE_BASE}/get-device-by-id/${id}`, { headers })
      )
    );
