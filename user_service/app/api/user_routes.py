...
from app.domain import CreateUserRequestDTO, GetAllUsersResponseDTO, GetUserResponseDTO
...
async def update_user(user_dto: CreateUserRequestDTO, user_id: int, db: Session = Depends(get_db), current_user = Depends(require_role("ADMIN"))):
    user_repo = UserRepository(db)
    user_service = UserService(user_repo)
